...
// même sortie que /products, avec cache
...
app.get('/products/:id', async (req, res) => {
  try {
    ...
    try {
      ...
      product.images = imgs.rows.map(r => ({
        id: r.id,
        url: `/images/${r.id}`,
...
// ------- routes admin (CRUD) : sans auth ici
...
app.post('/admin/products', upload.fields([{ name: 'image', maxCount: 1 }, { name: 'images', maxCount: 20 }]), async (req, res) => {
  ...
  try {
    ...
    const { name, description, price, stock, category_id, is_available, image_url, etat } = req.body;

    const ins = await client.query(
      `INSERT INTO products (name, description, price, stock, image_url, category_id, is_available, etat)
    ...
    console.error(e); 
    res.status(400).json({ error: e.message || 'create_error' });
  } finally {
    client.release();
  }
});
...
app.put('/admin/products/:id', upload.fields([{ name: 'image', maxCount: 1 }, { name: 'images', maxCount: 20 }]), async (req, res) => {
  ...
  try {
    ...
    await client.query('ROLLBACK');
    console.error(e); 
    res.status(400).json({ error: e.message || 'update_error' });
  } finally {
    client.release();
  }
});
...
app.delete('/admin/products/:id', async (req, res) => {
  try {
    ...
    console.error(e); 
    res.status(400).json({ error: e.message || 'delete_error' });
  }
});
